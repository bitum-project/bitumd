...
import (
	...
	"github.com/bitum-project/bitumd/bitumec/edwards"
	"github.com/bitum-project/bitumd/bitumec/secp256k1"
	"github.com/bitum-project/bitumd/bitumec/secp256k1/schnorr"
)
...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo bitumec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case bitumec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(bitumec.STEcdsaSecp256k1)
	case bitumec.STEd25519:
		pubKeyBytes[0] = byte(bitumec.STEd25519)
	case bitumec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(bitumec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == bitumec.STEcdsaSecp256k1 || algo == bitumec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) bitumec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch bitumec.SignatureType(suite) {
		case bitumec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, bitumec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, bitumec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, bitumec.STSchnorrSecp256k1)
...
// be 20 bytes.
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params, algo bitumec.SignatureType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case bitumec.STEcdsaSecp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) bitumec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return bitumec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return bitumec.STEd25519
	case net.PKHSchnorrAddrID:
		return bitumec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) bitumec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from bitumec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Bitum addresses
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) bitumec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) bitumec.SignatureType {
	return bitumec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) bitumec.SignatureType {
	return bitumec.STSchnorrSecp256k1
}
