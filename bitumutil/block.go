...
// Tx returns a wrapped transaction (bitumutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (bitumutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (bitumutil.Tx) for all
...
// instead provides easy access to wrapped versions (bitumutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (bitumutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (bitumwire.MsgTx) in the underlying wire.MsgBlock, however it
