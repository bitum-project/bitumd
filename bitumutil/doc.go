...
A Block defines a Bitum block that provides easier and more efficient
...
A Tx defines a Bitum transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Bitum address.  While the
most common type is a pay-to-pubkey-hash, Bitum already supports others and
...
To decode/encode an address:
	...
	addr, err := bitumutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
