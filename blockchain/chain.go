...
// BlockChain provides functions for working with the Bitum block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*bitumutil.Block
...
// StakeVersions is a condensed form of a bitumutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *bitumutil.Block) {
...
func (b *BlockChain) fetchMainChainBlockByNode(node *blockNode) (*bitumutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByNode(node *blockNode) (*bitumutil.Block, error) {
...
func (b *BlockChain) pushMainChainBlockCache(block *bitumutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *bitumutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*bitumutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *bitumutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *bitumutil.Block, view *UtxoViewpoint) error {
...
func countSpentRegularOutputs(block *bitumutil.Block) int {
...
func countSpentStakeOutputs(block *bitumutil.Block) int {
	var numSpent int
...
func countSpentOutputs(block *bitumutil.Block) int {
...
func (b *BlockChain) reorganizeChainInternal(targetTip *blockNode) error {
	...
	var nextBlockToDetach *bitumutil.Block
	...
	var prevBlockAttached *bitumutil.Block
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*bitumutil.Block, error) {
	node := b.index.LookupNode(hash)
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(height int64) (*bitumutil.Block, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *bitumutil.Block, *bitumutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *bitumutil.Block, *bitumutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*bitumutil.Block),
