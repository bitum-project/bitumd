...
func (s *SpendableOut) Amount() bitumutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      bitumutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   bitumutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := bitumutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() bitumutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) bitumutil.Amount {
	...
	return bitumutil.Amount(subsidy)
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy bitumutil.Amount, blockHeight uint32, numVotes uint16) bitumutil.Amount {
	powProportion := bitumutil.Amount(g.params.WorkRewardProportion)
	totalProportions := bitumutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := bitumutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * bitumutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) bitumutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := bitumutil.Amount(g.params.StakeRewardProportion)
	totalProportions := bitumutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy bitumutil.Amount, blockHeight uint32, numVotes uint16) bitumutil.Amount {
	devProportion := bitumutil.Amount(g.params.BlockTaxProportion)
	totalProportions := bitumutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := bitumutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * bitumutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy bitumutil.Amount) {
...
func PurchaseCommitmentScript(addr bitumutil.Address, amount, voteFeeLimit, revocationFeeLimit bitumutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee bitumutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / bitumutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := bitumutil.Amount(ticketTx.TxOut[0].Value)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*bitumutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, bitumutil.NewTx(tx))
	}
...
//
// This compact form is only used in Bitum to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee bitumutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee bitumutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := bitumutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = bitumutil.Amount(2)
	...
	var stakeTreeFees bitumutil.Amount
	for _, tx := range stakeTxns {
		...
		var inputSum, outputSum bitumutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += bitumutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += bitumutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount bitumutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy bitumutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := bitumutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += bitumutil.Amount(payout.Amount)
	}
