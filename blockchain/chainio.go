...
import (
	...
	"github.com/bitum-project/bitumd/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *bitumutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *bitumutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *bitumutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Bitum
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64, height uint32, index uint32) (int, error) {
	...
	// since in Bitum we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *bitumutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := bitumutil.NewBlock(b.chainParams.GenesisBlock)
...
// block for the provided node, deserialize it, and return a bitumutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*bitumutil.Block, error) {
	...
	block, err := bitumutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
