...
// CalcWork calculates a work value from difficulty bits.  Bitum increases
...
func (b *BlockChain) calcNextRequiredDifficulty(curNode *blockNode, newBlockTime time.Time) (uint32, error) {
	...
	if (curNode.height >= 19320 && curNode.height <= 19386) {
...
// weighted averages.
//
// NOTE: This is the original stake difficulty algorithm that was used at Bitum
// launch.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcNextRequiredStakeDifficultyV1(curNode *blockNode) (int64, error) {
	...
	// Bitum parameters, but might do weird things if you use custom
	// parameters.
	if curNode == nil ||
