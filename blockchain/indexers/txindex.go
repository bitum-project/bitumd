...
func dbAddTxIndexEntries(dbTx database.Tx, block *bitumutil.Block, blockID uint32) error {
	...
	addEntries := func(txns []*bitumutil.Tx, txLocs []wire.TxLoc, blockID uint32) error {
		offset := 0
...
// block (if they were valid) and every stake transaction in the passed block.
func dbRemoveTxIndexEntries(dbTx database.Tx, block *bitumutil.Block) error {
	removeEntries := func(txns []*bitumutil.Tx) error {
		for _, tx := range txns {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block, parent *bitumutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block, parent *bitumutil.Block, view *blockchain.UtxoViewpoint) error {
	// NOTE: The fact that the block can disapprove the regular tree of the
