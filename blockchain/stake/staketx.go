...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	...
	if isP2SH {
		addr, err = bitumutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = bitumutil.NewAddressPubKeyHash(hashBytes, params,
...
func AmountFromSStxPkScrCommitment(pkScript []byte) (bitumutil.Amount, error) {
	...
	return bitumutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
