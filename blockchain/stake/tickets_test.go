...
import (
	...
	"github.com/bitum-project/bitumd/blockchain/stake/internal/tickettreap"
...
func ticketsInBlock(bl *bitumutil.Block) []chainhash.Hash {
...
func TestTicketDBLongChain(t *testing.T) {
	...
	testBlockchain := make(map[int64]*bitumutil.Block, len(testBlockchainBytes))
	for k, v := range testBlockchainBytes {
		bl, err := bitumutil.NewBlockFromBytes(v)
		if err != nil {
