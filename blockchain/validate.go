...
func IsCoinBase(tx *bitumutil.Tx) bool {
...
// level util transaction as opposed to a raw wire transaction.
func IsExpired(tx *bitumutil.Tx, blockHeight int64) bool {
...
func IsFinalizedTransaction(tx *bitumutil.Tx, blockHeight int64, blockTime time.Time) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// Bitum is a quantity of atoms as defined by the AtomsPerCoin constant.
	//
...
func checkProofOfStake(block *bitumutil.Block, posLimit int64) error {
...
func checkBlockHeaderSanity(header *wire.BlockHeader, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	...
	if header.Height >= stakeValidationHeight && header.Height > 19386 {
		majority := uint16(1)
...
func checkBlockSanity(block *bitumutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	msgBlock := block.MsgBlock()
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *bitumutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
func (b *BlockChain) checkBlockPositional(block *bitumutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
// impossible.
func checkCoinbaseUniqueHeight(blockHeight int64, block *bitumutil.Block) error {
...
func (b *BlockChain) checkBlockContext(block *bitumutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
//
// Bitum: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*bitumutil.Tx, view *UtxoViewpoint) error {
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a vote.
func checkVoteInputs(subsidyCache *SubsidyCache, tx *bitumutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a revocation.
func checkRevocationInputs(tx *bitumutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
	ticketMaturity := int64(params.TicketMaturity)
	revokeHash := tx.Hash()
	msgTx := tx.MsgTx()
...
// owner of the Bitum and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *bitumutil.Tx, txHeight int64, view *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// Bitum stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Bitum general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	...
	for idx, txIn := range msgTx.TxIn {
		...
		// Bitum is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > bitumutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, bitumutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalAtomIn < lastAtomIn ||
			totalAtomIn > bitumutil.MaxAmount {
...
// txscript.
func CountSigOps(tx *bitumutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *bitumutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, view *UtxoViewpoint) (int, error) {
...
func (b *BlockChain) createLegacySeqLockView(block, parent *bitumutil.Block, view *UtxoViewpoint) (*UtxoViewpoint, error) {
...
func checkNumSigOps(tx *bitumutil.Tx, view *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*bitumutil.Tx, view *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeBaseAmounts(txs []*bitumutil.Tx, view *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*bitumutil.Tx, view *UtxoViewpoint) (bitumutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return bitumutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees bitumutil.Amount, node *blockNode, txs []*bitumutil.Tx, view *UtxoViewpoint, stxos *[]spentTxOut, txTree bool) error {
	...
	if txTree { //TxTreeRegular
		...
		if len(txs) == 0 && node.height >= b.chainParams.StakeValidationHeight && header.Height > 19386 {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *bitumutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckConnectBlockTemplate(block *bitumutil.Block) error {
	b.chainLock.Lock()
