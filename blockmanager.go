...
// blockMsg packages a Bitum block message and the peer it came from together
// so the block handler has access to that information.
type blockMsg struct {
	block *bitumutil.Block
	peer  *serverPeer
}
...
// headersMsg packages a Bitum headers message and the peer it came from
...
// txMsg packages a Bitum tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *bitumutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *bitumutil.Block) {
	var votesFromBlock []*bitumutil.Tx
	...
	var newVotes []*bitumutil.Tx
	var oldTickets []*bitumutil.Tx
	var oldRevocations []*bitumutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := bitumutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*bitumutil.Tx, 0,
	...
	var updatedTxTreeRegular []*bitumutil.Tx
...
func headerApprovesParent(header *wire.BlockHeader) bool {
	return bitumutil.IsFlagSet16(header.VoteBits, bitumutil.BlockValid)
}
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTNewTipBlockChecked:
		...
		block, ok := notification.Data.(*bitumutil.Block)
		if !ok {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*bitumutil.Block)
		if !ok {
		...
		handleConnectedBlockTxns := func(txns []*bitumutil.Tx) {
			for _, tx := range txns {
	...
	case blockchain.NTBlockDisconnected:
		...
		handleDisconnectedBlockTxns := func(txns []*bitumutil.Tx) {
			for _, tx := range txns {
				_, err := txMemPool.MaybeAcceptTransaction(tx, false, true)
				if err != nil && !isDoubleSpendOrDuplicateError(err) {
					txMemPool.RemoveTransaction(tx, true)
...
// queue.
func (b *blockManager) QueueTx(tx *bitumutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *bitumutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *bitumutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *bitumutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*bitumutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (bitumutil.Amount, error) {
...
// newBlockManager returns a new Bitum block manager.
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		bitumdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		bitumdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		bitumdLog.Infof("Creating block database in memory.")
	...
	bitumdLog.Infof("Loading block database from '%s'", dbPath)
	...
	bitumdLog.Info("Block database loaded")
	return db, nil
}
