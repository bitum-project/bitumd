...
func main() {
	...
	method := args[0]
	usageFlags, err := bitumjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := bitumjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// bitumjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(bitumjson.Error); ok {
		...
		// The error is not a bitumjson.Error and this really should not
	...
	marshalledJSON, err := bitumjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
