...
const (
	...
	unusableFlags = bitumjson.UFWebsocketOnly | bitumjson.UFNotification
)

var (
	bitumdHomeDir          = bitumutil.AppDataDir("bitumd", false)
	bitumctlHomeDir        = bitumutil.AppDataDir("bitumctl", false)
	bitumwalletHomeDir     = bitumutil.AppDataDir("bitumwallet", false)
	defaultConfigFile      = filepath.Join(bitumctlHomeDir, "bitumctl.conf")
...
func listCommands() {
	...
	cmdMethods := bitumjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := bitumjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := bitumjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&bitumjson.UFWalletOnly != 0 {
...
// config defines the configuration options for bitumctl.
//
...
// For this it tries to read the bitumd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing bitumd conf file at the default
	// path to extract the details from.
	bitumdConfigPath := filepath.Join(bitumdHomeDir, "bitumd.conf")
	if !fileExists(bitumdConfigPath) {
		return nil
	}

	// Read bitumd.conf from its default path
	bitumdConfigFile, err := os.Open(bitumdConfigPath)
	if err != nil {
		return err
	}
	defer bitumdConfigFile.Close()
	content, err := ioutil.ReadAll(bitumdConfigFile)
	if err != nil {
		return err
	}
