...
import (
	...
	_ "github.com/bitum-project/bitumd/database/ffldb"
	"github.com/bitum-project/bitumd/internal/version"
	"github.com/bitum-project/bitumd/mempool"
	"github.com/bitum-project/bitumd/sampleconfig"
	"github.com/bitum-project/bitumd/bitumutil"
)

const (
	defaultConfigFilename        = "bitumd.conf"
...
var (
	defaultHomeDir     = bitumutil.AppDataDir("bitumd", false)
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in BITUM/kB to be considered a non-zero fee."`
	...
	AltDNSNames          []string      `long:"altdnsnames" description:"Specify additional dns names to use when generating the rpc server certificate" env:"BITUMD_ALT_DNSNAMES" env-delim:","`
	onionlookup          func(string) ([]net.IP, error)
	lookup               func(string) ([]net.IP, error)
	oniondial            func(string, string) (net.Conn, error)
	dial                 func(string, string) (net.Conn, error)
	miningAddrs          []bitumutil.Address
...
// createDefaultConfig copies the file sample-bitumd.conf to the given destination path,
...
//
// The above results in bitumd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for bitumd if specified. Since the home
	...
	cfg.minRelayTxFee, err = bitumutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]bitumutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := bitumutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			bitumdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		bitumdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// bitumdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func bitumdDial(network, addr string) (net.Conn, error) {
...
// bitumdLookup returns the correct DNS lookup function to use depending on the
...
func bitumdLookup(host string) ([]net.IP, error) {
