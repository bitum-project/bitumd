...
type cpuminerConfig struct {
	...
	ProcessBlock func(*bitumutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *bitumutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		bitumutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := bitumutil.NewBlock(template.Block)
			m.submitBlock(block)
