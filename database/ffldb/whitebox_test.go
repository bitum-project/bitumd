...
// a slice of them.
func loadBlocks(t *testing.T, dataFile string, network wire.CurrencyNet) ([]*bitumutil.Block, error) {
	...
	blocks := make([]*bitumutil.Block, 169)
	for i := 0; i <= 168; i++ {
		bl, err := bitumutil.NewBlockFromBytes(blockChain[int64(i)])
		if err != nil {
