...
bitumd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between bitumd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, bitumd intentionally splits the wallet and chain
  ...
  talking directly to bitumd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [bitumwallet](https://github.com/bitum-project/bitumwallet).
* bitumd is secure by default which means that the RPC connection is TLS-enabled
  by default
* bitumd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for bitumd RPC and are used by applications
such as [bitumwallet](https://github.com/bitum-project/bitumwallet) for inter-process
communication with bitumd.  The websocket connection endpoint for bitumd is
...
during the development of bitumwallet.
...
The bitumd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
bitumd is `wss://your_ip_or_domain:9209/ws`.
...
to a bitumd RPC server:

* **rpcuser** is the full-access username configured for the bitumd RPC server
* **rpcpass** is the full-access password configured for the bitumd RPC server
* **rpclimituser** is the limited username configured for the bitumd RPC server
* **rpclimitpass** is the limited password configured for the bitumd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the bitumd
  server is configured with.  It is automatically generated by bitumd and placed
  in the bitumd home directory (which is typically `%LOCALAPPDATA%\Dcrd` on
  Windows and `~/.bitumd` on POSIX-like OSes)

**NOTE:** As mentioned above, bitumd is secure by default which means the RPC
...
The bitumd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
bitumd comes with a separate utility named `bitumctl` which can be used to issue
these RPC commands via HTTP POST requests to bitumd after configuring it with the
...
API compatible with the original bitumd client.
...
|25|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since bitumd does not have the wallet integrated to provide payment addresses, bitumd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|28|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: bitumd does not yet implement the `allowhighfees` parameter, so it has no effect|
|29|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since bitumd does not have the wallet integrated to provide payment addresses, bitumd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|30|[stop](#stop)|N|Shutdown bitumd.|
|31|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|32|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since bitumd does not have a wallet integrated, bitumd will only return whether the address is valid or not.|
...
|36|[getcurrentnet](#getcurrentnet)|Y|Get Bitum network bitumd is running on.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9208", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/bitumd:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since bitumd does not have the wallet integrated to provide payment addresses, bitumd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|NOTE: Since bitumd does not have the wallet integrated to provide payment addresses, bitumd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|bitumd does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown bitumd.|
|Returns|`"bitumd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. `address`: `(string, required)` Bitum address.|
|Description|Verify an address is valid.|
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the Bitum address validated.<br /><br />`{"isvalid": true or false,"address": "bitumaddress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For bitumd this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|Dcrd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for bitumd.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `BITUMD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR BITUMD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get Bitum network bitumd is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`bitumaddress`: `(string)` the Bitum address.<br /><br />`["bitumaddress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this bitumd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
bitumd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll bitumd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/bitum-project/bitumd/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/bitum-project/bitumd/tree/master/rpcclient) package to connect with
a bitumd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by bitumd when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/bitum-project/bitumd/tree/master/rpcclient) package to connect with
a bitumd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/bitum-project/bitumd/tree/master/rpcclient) package to connect with
a bitumd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a bitumd instance,
...
// generated by bitumd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/bitumd/appdata/rpc.cert');
