...
type EstimatorConfig struct {
	...
	// it's ignored.
	ExtraBucketFee bitumutil.Amount
...
func (stats *Estimator) EstimateFee(targetConfs int32) (bitumutil.Amount, error) {
	stats.lock.RLock()
	...
	return bitumutil.Amount(rate), nil
}
...
func (stats *Estimator) AddMemPoolTransaction(txHash *chainhash.Hash, fee, size int64, txType stake.TxType) {
	...
	// minimum at values less than 0.001 BITUM/KB. This is needed because due to
...
func (stats *Estimator) ProcessBlock(block *bitumutil.Block) error {
	stats.lock.Lock()
	defer stats.lock.Unlock()
