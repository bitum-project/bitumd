...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	parentFP := bitumutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a bitumec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*secp256k1.PublicKey, error) {
	return secp256k1.ParsePubKey(k.pubKeyBytes())
}

// ECPrivKey converts the extended key to a bitumec private key and returns it.
...
// Address converts the extended key to a standard Bitum pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*bitumutil.AddressPubKeyHash, error) {
	pkHash := bitumutil.Hash160(k.pubKeyBytes())
	return bitumutil.NewAddressPubKeyHash(pkHash, net, bitumec.STEcdsaSecp256k1)
}
