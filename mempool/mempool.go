...
import (
	...
	"github.com/bitum-project/bitumd/blockchain/indexers"
	...
	"github.com/bitum-project/bitumd/mining"
...
const (
	...
	// BITUM/kB, this results in a maximum allowed high fee of 1 BITUM/kB.
...
type Config struct {
	...
	FetchUtxoView func(*bitumutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*bitumutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*bitumutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in BITUM/kB to be
	// considered a non-zero fee.
	MinRelayTxFee bitumutil.Amount
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*bitumutil.Tx
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*bitumutil.Tx
	outpoints     map[wire.OutPoint]*bitumutil.Tx
...
func (mp *TxPool) insertVote(ssgen *bitumutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := bitumutil.IsFlagSet16(voteBits, bitumutil.BlockValid)
...
func (mp *TxPool) removeOrphan(tx *bitumutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *bitumutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *bitumutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*bitumutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *bitumutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *bitumutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *bitumutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *bitumutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *bitumutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *bitumutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *bitumutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*bitumutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
// It should also set the bitumutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *bitumutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *bitumutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *bitumutil.Tx) []*bitumutil.Tx {
	var acceptedTxns []*bitumutil.Tx
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*bitumutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *bitumutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*bitumutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*bitumutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*bitumjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*bitumjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &bitumjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              bitumutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*bitumutil.Tx),
		orphansByPrev: make(map[wire.OutPoint]map[chainhash.Hash]*bitumutil.Tx),
		outpoints:     make(map[wire.OutPoint]*bitumutil.Tx),
