...
type fakeChain struct {
	...
	blocks         map[chainhash.Hash]*bitumutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *bitumutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*bitumutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *bitumutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *bitumutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
func (s *fakeChain) AddFakeUtxoMedianTime(tx *bitumutil.Tx, txOutIdx uint32, medianTime time.Time) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *bitumutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   bitumutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// passed in.
func (p *poolHarness) GetScript(addr bitumutil.Address) ([]byte, error) {
...
func (p *poolHarness) GetKey(addr bitumutil.Address) (chainec.PrivateKey, bool, error) {
	return p.signKey, true, nil
}

// AddFakeUTXO creates a fake mined uxto for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *bitumutil.Tx, blockHeight int64) {
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*bitumutil.Tx, error) {
...
// testing.
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*bitumutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput bitumutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*bitumutil.Tx, error) {
	txChain := make([]*bitumutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, bitumutil.NewTx(tx))
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*bitumutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchase(sourceTx *bitumutil.Tx, cost int64) (*bitumutil.Tx, error) {
	ticketfee := bitumutil.Amount(singleInputTicketSize)
	ticketPrice := bitumutil.Amount(cost)
	...
	change := bitumutil.Amount(sourceTx.MsgTx().TxOut[0].Value) -
...
func (p *poolHarness) CreateVote(ticket *bitumutil.Tx) (*bitumutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, vote, inputToSign,
		...
		p, vote.TxIn[inputToSign].SignatureScript, bitumec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return bitumutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *bitumutil.Tx) (*bitumutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, revocation, inputToSign,
		redeemTicketScript, txscript.SigHashAll, p,
		p, revocation.TxIn[inputToSign].SignatureScript, bitumec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return bitumutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := bitumutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*bitumutil.Block),
...
func testPoolMembership(tc *testContext, tx *bitumutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*bitumutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   bitumutil.Amount(5000000000),
...
func TestSequenceLockAcceptance(t *testing.T) {
	...
	for _, acceptSeqLocks := range []bool{false, true} {
		...
		for i, test := range tests {
			...
			inputTx := bitumutil.NewTx(inputMsgTx)
