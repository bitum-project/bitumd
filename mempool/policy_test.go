...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee bitumutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: bitumutil.MaxAmount, Version: 0, PkScript: pkScript},
			bitumutil.MaxAmount,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := bitumutil.NewAddressPubKeyHash(addrHash[:],
		&chaincfg.RegNetParams, bitumec.STEcdsaSecp256k1)
	if err != nil {
