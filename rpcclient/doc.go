...
/*
Package rpcclient implements a websocket-enabled Bitum JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Bitum RPC server that uses a mostly btcd/bitcoin core
style Bitum JSON-RPC API.  This client has been tested with bitumd
(https://github.com/bitum-project/bitumd) and bitumwallet
(https://github.com/bitum-project/bitumwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, bitumd and
bitumwallet provide a websocket interface that is more efficient than the standard
...
bitumd or bitumwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by bitumd and
bitumwallet only uses a single connection that remains open and allows
...
All notifications provided by bitumd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by bitumd (and
bitumwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that bitumd intentionally separates the wallet
functionality into a separate process named bitumwallet.  This means if you are
connected to the bitumd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, bitumwallet provides pass through treatment
...
detected by type asserting the error in a *bitumjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*bitumjson.RPCError); ok {
  		switch jerr.Code {
  		case bitumjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - bitumdwebsockets
   ...
   block count
 - bitumwalletwebsockets
   Connects to a bitumwallet RPC server using TLS-secured websockets, registers
