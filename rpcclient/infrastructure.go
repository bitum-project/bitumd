...
// Client represents a Bitum RPC client which allows easy access to the
// various RPC methods available on a Bitum RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *bitumjson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *bitumjson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *bitumjson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *bitumjson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *bitumjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *bitumjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *bitumjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled bitumjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := bitumjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := bitumjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
