...
var (
	...
	// data.
	gbtCoinbaseAux = &bitumjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &bitumjson.RPCError{
	...
	ErrRPCNoWallet = &bitumjson.RPCError{
		Code:    bitumjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which bitumd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of bitumwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *bitumjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return bitumjson.NewRPCError(bitumjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *bitumjson.RPCError {
	return bitumjson.NewRPCError(bitumjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in bitumwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.CreateRawTransactionCmd)
	...
	for _, input := range c.Inputs {
		...
		if input.Amount > 0 {
			amt, err := bitumutil.NewAmount(input.Amount)
			if err != nil {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > bitumutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, bitumutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := bitumutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := bitumutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := bitumutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			bitumutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > bitumutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, bitumutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = bitumutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.CreateRawSSRtxCmd)
	...
	var feeAmt bitumutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = bitumutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > bitumutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, bitumutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []bitumjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]bitumjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = bitumutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &bitumjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []bitumjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]bitumjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout bitumjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = bitumutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = bitumjson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*bitumjson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.DecodeRawTransactionCmd)
	...
	txReply := bitumjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.DecodeScriptCmd)
	...
	p2sh, err := bitumutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := bitumjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateSmartFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.EstimateSmartFeeCmd)

	mode := bitumjson.EstimateSmartFeeConservative
	if c.Mode != nil {
		mode = *c.Mode
	}

	if mode != bitumjson.EstimateSmartFeeConservative {
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := bitumutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &bitumjson.EstimateStakeDiffResult{
		Min:      bitumutil.Amount(min).ToCoin(),
		Max:      bitumutil.Amount(max).ToCoin(),
		Expected: bitumutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := bitumutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.ExistsAddressesCmd)
	addresses := make([]bitumutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := bitumutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ExistsMissedTicketsCmd)

	hashes, err := bitumjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the Bitum server
	...
	results := make([]*bitumjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result bitumjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = bitumjson.Bool(peer.Connected())
		...
		addrs := make([]bitumjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr bitumjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / bitumutil.AtomsPerCoin
	blockReply := bitumjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]bitumjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]bitumjson.TxRawResult, len(stxns))
...
func handleGetBlockchainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	response := bitumjson.GetBlockChainInfoResult{
		Chain:                params.Name,
		Blocks:               best.Height,
		Headers:              best.Height,
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &bitumjson.RPCError{
			Code: bitumjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetBlockHeaderCmd)
	...
	blockHeaderReply := bitumjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		MerkleRoot:    blockHeader.MerkleRoot.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         bitumutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetBlockSubsidyCmd)
	...
	rep := bitumjson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
// state as a bitumjson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*bitumjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]bitumjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := bitumjson.GetBlockTemplateResultTx{
	...
	stransactions := make([]bitumjson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := bitumjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *bitumjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
...
// reasons.
// TODO Bitum pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *bitumjson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetChainTips(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := make([]bitumjson.GetChainTipsResult, 0, len(chainTips))
	for _, tip := range chainTips {
		result = append(result, bitumjson.GetChainTipsResult{
			Height:    tip.Height,
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.server.cfIndex == nil {
		return nil, &bitumjson.RPCError{
			Code:    bitumjson.ErrRPCNoCFIndex,
			Message: "Compact filters must be enabled for this command",
		}
	}

	c := cmd.(*bitumjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetHeadersCmd)
	blockLocators, err := bitumjson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *bitumjson.RPCError
		return nil, err
	}
	...
	return &bitumjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &bitumjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := bitumjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := bitumjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &bitumjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*bitumjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &bitumjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch bitumjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case bitumjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case bitumjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case bitumjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case bitumjson.GRMRevocations:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSRtx
		case bitumjson.GRMAll:
			// Nothing to do
		default:
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &bitumjson.RPCError{
			Code:    bitumjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := bitumutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := bitumutil.Amount(nextSdiff)

	sDiffResult := &bitumjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []bitumjson.VersionCount {
	sorted := make([]bitumjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, bitumjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*bitumjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := bitumjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]bitumjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := bitumjson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.GetStakeVersionsCmd)
	...
	result := bitumjson.GetStakeVersionsResult{
		StakeVersions: make([]bitumjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := bitumjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]bitumjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				bitumjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*bitumjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := bitumjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var txFromMempool *bitumutil.Tx
	if includeMempool {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &bitumjson.RPCError{
				Code: bitumjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &bitumjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         bitumutil.Amount(value).ToUnit(bitumutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: bitumjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return bitumjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return bitumjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]bitumjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]bitumjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := bitumutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]bitumjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &bitumjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     bitumutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr bitumutil.Address, numToSkip, numRequested uint32) ([]*bitumutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bitumjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]bitumjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.SendRawTransactionCmd)
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.SetGenerateCmd)
...
func min(s []bitumutil.Amount) bitumutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []bitumutil.Amount) bitumutil.Amount {
	max := bitumutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []bitumutil.Amount) bitumutil.Amount {
	sum := bitumutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / bitumutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []bitumutil.Amount) bitumutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func stdDev(s []bitumutil.Amount) bitumutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := bitumutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *bitumjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]bitumutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (bitumutil.Amount(txD.Fee)) * 1000 /
				bitumutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &bitumjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *bitumutil.Tx) bitumutil.Amount {
	var in bitumutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += bitumutil.Amount(txIn.ValueIn)
	}
	var out bitumutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += bitumutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / bitumutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*bitumjson.FeeInfoBlock, error) {
	...
	txFees := make([]bitumutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*bitumjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []bitumutil.Amount
	for i := range hashes {
	...
	return &bitumjson.FeeInfoWindow{
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []bitumjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []bitumjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &bitumjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.TicketsForAddressCmd)
	...
	reply := &bitumjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return bitumutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange bitumjson.FeeInfoRange
	...
	feeInfoRange = bitumjson.FeeInfoRange{
	...
	return &bitumjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.ValidateAddressCmd)
	result := bitumjson.ValidateAddressChainResult{}
	addr, err := bitumutil.DecodeAddress(c.Address)
...
func handleVerifyChain(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.VerifyChainCmd)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bitumjson.VerifyMessageCmd)
	...
	address, err := bitumutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]bitumjson.VersionResult{
		"bitumdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, bitumjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *bitumjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(bitumjson.Error); ok &&
			jerr.Code == bitumjson.ErrUnregisteredMethod {
			parsedCmd.err = bitumjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *bitumjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	if replyErr != nil {
		if jErr, ok := replyErr.(*bitumjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return bitumjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *bitumjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = bitumjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = bitumjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = bitumjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="bitumd RPC"`)
