...
type memWallet struct {
	...
	coinbaseAddr bitumutil.Address
	...
	addrs map[uint32]bitumutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]bitumutil.Address)
...
// SetRPCClient saves the passed rpc connection to bitumd as the wallet's
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*bitumutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := bitumutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          bitumutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (bitumutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []bitumutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt bitumutil.Amount, feeRate bitumutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := bitumutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate bitumutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate bitumutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt bitumutil.Amount
	for _, output := range outputs {
		outputAmt += bitumutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() bitumutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance bitumutil.Amount
...
func keyToAddr(key *secp256k1.PrivateKey, net *chaincfg.Params) (bitumutil.Address, error) {
	pubKey := (*secp256k1.PublicKey)(&key.PublicKey)
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := bitumutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
