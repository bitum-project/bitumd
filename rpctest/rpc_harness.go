...
// Harness fully encapsulates an active bitumd process to provide a unified
// platform for creating rpc driven integration tests involving bitumd. The
// active bitumd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active bitumd process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// wallet.
	filterAddrs := []bitumutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure bitumd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created bitumd
...
func (h *Harness) NewAddress() (bitumutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() bitumutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut, feeRate bitumutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut, feeRate bitumutil.Amount) (*wire.MsgTx, error) {
