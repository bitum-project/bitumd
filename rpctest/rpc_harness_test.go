...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt bitumutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(bitumutil.Amount(5 * bitumutil.AtomsPerCoin))
	...
	txid = genSpend(bitumutil.Amount(5000 * bitumutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(bitumjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(bitumjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := bitumutil.Amount(5 * 300 * bitumutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = bitumutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := bitumutil.Amount(50 * bitumutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := bitumutil.Amount(numMatureOutputs * 300 * bitumutil.AtomsPerCoin)
