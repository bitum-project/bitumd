...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *bitumutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *bitumutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *bitumutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a bitumutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string) {
	a, err := bitumutil.DecodeAddress(s)
...
func (f *wsClientFilter) existsAddress(a bitumutil.Address) bool {
	switch a := a.(type) {
	case *bitumutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected bitumutil.Block
type notificationBlockDisconnected bitumutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *bitumutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *bitumutil.Block) {
	...
	ntfn := bitumjson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := bitumjson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *bitumutil.Block) {
	...
	ntfn := bitumjson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := bitumjson.MarshalCmd("1.0", nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := bitumjson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := bitumjson.MarshalCmd("1.0", nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := bitumjson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := bitumjson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := bitumjson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(clients map[chan struct{}]*wsClient, sdnd *StakeDifficultyNtfnData) {
	// Notify interested websocket clients about the connected block.
	ntfn := bitumjson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *bitumutil.Tx) {
	...
	ntfn := bitumjson.NewTxAcceptedNtfn(txHashStr,
		bitumutil.Amount(amount).ToCoin())
	marshalledJSON, err := bitumjson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *bitumjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = bitumjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = bitumjson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *bitumutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := bitumjson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := bitumjson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &bitumjson.RPCError{
					Code:    bitumjson.ErrRPCParse.Code,
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &bitumjson.RPCError{
					Code:    bitumjson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*bitumjson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &bitumjson.RPCError{
						Code:    bitumjson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = bitumjson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
			...
			if err == nil {
				...
				if len(batchedRequests) > 0 {
					...
					for _, entry := range batchedRequests {
						...
						if err != nil || math.IsNaN(string(reply)) || reply == nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bitumjson.HelpCmd)
	if !ok {
		return nil, bitumjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &bitumjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bitumjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, bitumjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *bitumutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bitumjson.RescanCmd)
	if !ok {
		return nil, bitumjson.ErrRPCInternal
	}
	...
	if filter == nil {
		return nil, &bitumjson.RPCError{
			Code:    bitumjson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := bitumjson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]bitumjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &bitumjson.RPCError{
				Code:    bitumjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, bitumjson.RescannedBlock{
	...
	return &bitumjson.RescanResult{DiscoveredData: discoveredData}, nil
}
