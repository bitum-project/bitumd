...
// FileContents is a string containing the commented example config for bitumd.
...
; The default is ~/.bitumd/data on POSIX OSes, $LOCALAPPDATA/Dcrd/data on Windows,
; ~/Library/Application Support/Dcrd/data on macOS, and $homed/bitumd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.bitumd/data                            ; Unix
...
; line.  bitumd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, bitumd will query DNS
...
; Disable DNS seeding for peers.  By default, when bitumd starts, it will use
...
; which is used to control and query information from a running bitumd process.
;
...
; log level for individual subsystems.  Use bitumd --debuglevel=show to list
