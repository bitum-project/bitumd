...
import (
	...
	"github.com/bitum-project/bitumd/fees"
...
var (
	...
	// ourselves to other Bitum peers.
	userAgentName = "bitumd"
...
// broadcastMsg provides the ability to house a Bitum message to be broadcast
...
// server provides a Bitum server for handling communications to and from
// Bitum peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a bitumutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := bitumutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a bitumutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := bitumutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*bitumutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, bitumdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of bitumjson.GetAddedNodeInfoResult structures
...
// newServer returns a new bitumd server configured to listen on addr for the
// Bitum network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, dataDir string, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, bitumdLookup)
	...
	feC := fees.EstimatorConfig{
		...
		MaxBucketFee: bitumutil.Amount(fees.DefaultMaxBucketFeeMultiplier) * cfg.minRelayTxFee,
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The bitumdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := bitumdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
