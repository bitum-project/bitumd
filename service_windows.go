...
const (
	// svcName is the name of bitumd service.
	svcName = "bitumdsvc"
	...
	svcDesc = "Downloads and stays synchronized with the Bitum block " +
...
// logServiceStartOfDay logs information about bitumd when the main server has
...
// bitumdService houses the main service handler which handles all service
// updates and launching bitumdMain.
type bitumdService struct{}
...
// long-running bitumdMain (which is the real meat of bitumd), handles service
// change requests, and notifies the service control manager of changes.
func (s *bitumdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start bitumdMain in a separate goroutine so the service can start
...
// installService attempts to install the bitumd service.  Typically this should
...
// removeService attempts to uninstall the bitumd service.  Typically this should
...
// startService attempts to start the bitumd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &bitumdService{})
	if err != nil {
