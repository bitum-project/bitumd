...
// This example demonstrates creating a script which pays to a Bitum address.
// It also prints the created script hex and uses the DisasmString function to
// display the disassembled script.
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a bitumutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "DsSej1qR3Fyc8kV176DCh9n9cY9nqf9Quxk"
	address, err := bitumutil.DecodeAddress(addressStr)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	addr, err := bitumutil.NewAddressPubKeyHash(pubKeyHash,
		&chaincfg.MainNetParams, bitumec.STEcdsaSecp256k1)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	// contains a single output that pays to address in the amount of 1 BITUM.
	...
	lookupKey := func(a bitumutil.Address) (chainec.PrivateKey, bool, error) {
