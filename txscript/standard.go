...
func payToPubKeyHashEdwardsScript(pubKeyHash []byte) ([]byte, error) {
	edwardsData := []byte{byte(bitumec.STEd25519)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
func payToPubKeyHashSchnorrScript(pubKeyHash []byte) ([]byte, error) {
	schnorrData := []byte{byte(bitumec.STSchnorrSecp256k1)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
// valid SStxs.
func PayToSStx(addr bitumutil.Address) ([]byte, error) {
	...
	case *bitumutil.AddressPubKeyHash:
		...
		if addr.DSA(addr.Net()) != bitumec.STEcdsaSecp256k1 {
...
// valid SStxs.
func PayToSStxChange(addr bitumutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSGen txs.
func PayToSSGen(addr bitumutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSRtx.
func PayToSSRtx(addr bitumutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// an SStx.
func GenerateSStxAddrPush(addr bitumutil.Address, amount bitumutil.Amount, limits uint16) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// specified address.
func PayToAddrScript(addr bitumutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *bitumutil.AddressPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
	...
	case *bitumutil.AddressEdwardsPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *bitumutil.AddressSecSchnorrPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*bitumutil.AddressSecpPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []bitumutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := bitumutil.NewAddressPubKeyHash(pops[2].data,
			chainParams, bitumec.STEcdsaSecp256k1)
		if err == nil {
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := bitumutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case bitumec.STEd25519:
			addr, err = bitumutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case bitumec.STSchnorrSecp256k1:
			addr, err = bitumutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []bitumutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := bitumutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]bitumutil.Address, 0, numPubKeys)
...
func ExtractPkScriptAltSigType(pkScript []byte) (bitumec.SignatureType, error) {
	pops, err := parseScript(pkScript)
	if err != nil {
		return 0, err
	}
	...
	val := bitumec.SignatureType(valInt)
	switch val {
	case bitumec.STEd25519:
		return val, nil
	case bitumec.STSchnorrSecp256k1:
...
//
// NOTE: Atomic swaps are not considered standard script types by the bitumd
