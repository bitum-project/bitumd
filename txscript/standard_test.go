...
// newAddressPubKey returns a new bitumutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) bitumutil.Address {
	pubkey, err := secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := bitumutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new bitumutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) bitumutil.Address {
	addr, err := bitumutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
...
// newAddressScriptHash returns a new bitumutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) bitumutil.Address {
	addr, err := bitumutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the bitumutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the bitumutil.Address interface.
...
// DSA returns -1.
func (b *bogusAddress) DSA(chainParams *chaincfg.Params) bitumec.SignatureType {
	return -1
}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := bitumutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := bitumutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := bitumutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := bitumutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Bitum this would
		// be uncompressed, but standard for Bitum is 33 byte
		...
		{(*bitumutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bitumutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*bitumutil.AddressSecpPubKey)(nil), "", errUnsupportedAddress},
		{(*bitumutil.AddressEdwardsPubKey)(nil), "", errUnsupportedAddress},
		{(*bitumutil.AddressSecSchnorrPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := bitumutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*bitumutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*bitumutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Bitum.
			[]*bitumutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*bitumutil.AddressSecpPubKey),
			},
			1,
...
func TestGenerateSStxAddrPush(t *testing.T) {
	...
	for _, test := range tests {
		addr, err := bitumutil.DecodeAddress(test.addrStr)
		if err != nil {
