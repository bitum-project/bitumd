...
/*
Package wire implements the Bitum wire protocol.

For the complete details of the Bitum protocol, see the official wiki entry
...
supported Bitum messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Bitum Message Overview

The Bitum protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which Bitum network it is a part of, its type, how big it is, and a checksum
...
of the currently supported Bitum messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using Bitum encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the Bitum messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official Bitum protocol wiki entry at
...
and write Bitum messages.  The following sections provide a quick overview of
...
The Bitum network is a magic number which is used to identify the start of a
message and which Bitum network the message applies to.  This package provides
...
As discussed in the Bitum message overview section, this package reads
and writes Bitum messages using a generic interface named Message.  In
...
In order to unmarshall Bitum messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a Bitum peer.  Example syntax is:

	// Reads and validates the next Bitum message from conn using the
	// protocol version pver and the Bitum network btcnet.  The returns
...
In order to marshall Bitum messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a Bitum peer.  Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the Bitum network btcnet.  The return is a possible
	// error.
