...
// OutPoint defines a Bitum data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new Bitum transaction outpoint point with the
...
// NewTxIn returns a new Bitum transaction input with the provided
...
// NewTxOut returns a new Bitum transaction output with the provided
...
// MsgTx implements the Message interface and represents a Bitum tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Bitum
...
// BtcDecode decodes r using the Bitum protocol encoding into the receiver.
...
// in that BtcDecode decodes from the Bitum wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the Bitum protocol encoding.
// This is part of the Message interface implementation.
...
// encodes the transaction to the Bitum wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new Bitum tx message that conforms to the Message
...
// WriteOutPoint encodes op to the Bitum protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the Bitum protocol encoding for a transaction
...
// writeTxWitness encodes ti to the Bitum protocol encoding for a transaction
...
// writeTxOut encodes to into the Bitum protocol encoding for a transaction
// output (TxOut) to w.
